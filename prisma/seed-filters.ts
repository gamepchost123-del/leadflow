...
const STAFFING_DOMAIN_PATTERNS = [
  ...
  // Note: For wildcard matching, we'll store simple patterns and use `includes` logic in the app
  'personeel', 'vacaturebank', 'vacaturesbank', 'jobboard', 'banensite', 'banen.'
];

async function seed() {
  console.log('Seeding FilterRules...');
  ...
  await insertRules('KEYWORD', 'STAFFING_DOMAIN_PATTERN', STAFFING_DOMAIN_PATTERNS); // Using KEYWORD type for domain substring matches

  console.log(`Seed complete. Inserted ${inserted} rules (skipped ${skipped} existing).`);
}
